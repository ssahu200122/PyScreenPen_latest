...
class Canvas(QWidget):
    ...
    def move_selection(self, delta):
        ...
        for i in self.selected_indices:
            ...
            if "fill_path" in stroke: stroke["fill_path"].translate(delta)
    ...
    def snap_to_shape(self):
        ...
        if shape_type:
            self.snapped_shape = {
                ...
                "is_filled": state.is_fill_enabled, "fill_color": state.current_fill_color, "fill_path": detected_path
            }
    ...
    # --- TABLET PRESSURE HANDLER ---
    def tabletEvent(self, event):
        self.current_pressure = event.pressure()
        if self.current_pressure == 0: self.current_pressure = 0.5
        event.ignore() 
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
                    ...
                    if rect.contains(event.position()):
                        ...
                        for k, stroke in enumerate(self.original_selected_strokes):
                            if stroke["type"] != "text": 
                                ...
                                if "fill_path" in stroke: stroke["fill_path"] = QPainterPath(self.strokes[self.selected_indices[k]]["fill_path"])
                        return
            ...
            self.current_stroke = {
                ...
                "pressures": [self.current_pressure],
                "is_filled": state.is_fill_enabled, "fill_color": state.current_fill_color,
                "fill_path": QPainterPath() # Separate path for fill
            }
            if self.current_stroke["type"] in ["pen", "highlighter", "eraser", "laser_pen"]:
                self.current_stroke["path"].moveTo(self.last_pos)
                self.current_stroke["fill_path"].moveTo(self.last_pos)
    ...
    def mouseMoveEvent(self, event):
            ...
            if self.transform_mode == "scale":
                ...
                for idx, orig_stroke in enumerate(self.original_selected_strokes):
                    ...
                    if orig_stroke["type"] == "text":
                        ...
                        if "fill_path" in orig_stroke: self.strokes[real_idx]["fill_path"] = transform.map(orig_stroke["fill_path"])
        ...
        if self.is_scaling_shape and self.snapped_shape:
            if self.snapped_shape["shape_type"] == "line":
                ...
                if self.snapped_shape.get("is_filled"): self.snapped_shape["fill_path"] = self.snapped_shape["path"]
            self.update(); return 
        ...
        if self.is_drawing:
            ...
            if "pen" in self.active_tool:
                ...
                if self.current_stroke and "pressures" in self.current_stroke:
                    self.current_stroke["pressures"].append(self.current_pressure)
                    if len(self.current_stroke["pressures"]) > 2:
                        self.current_stroke["path"] = self.generate_stroke_path(self.current_points, self.current_stroke["pressures"], self.current_stroke["size"])
                        self.current_stroke["is_variable"] = True
                
                dist = (pos - self.last_pos).manhattanLength()
                if dist > 2.0:
                    self.shape_hold_timer.start()
            ...
            if self.current_stroke:
                ...
                if st_type in ["pen", "highlighter", "eraser", "laser_pen"]:
                    ...
                    self.current_stroke["path"].quadTo(self.last_pos, QPointF(mid_x, mid_y)); 
                    if "fill_path" in self.current_stroke: self.current_stroke["fill_path"].quadTo(self.last_pos, QPointF(mid_x, mid_y))
                    self.last_pos = pos
                else: self.current_stroke["end"] = pos.toPoint()
            elif self.current_stroke and self.current_stroke.get("is_variable", False):
                 # Update fill path for variable stroke (centerline)
                 if "fill_path" in self.current_stroke: self.current_stroke["fill_path"].lineTo(pos)
                 self.last_pos = pos
        ...
        if event.button() == Qt.LeftButton and self.is_drawing:
            ...
            if self.current_stroke:
                ...
                if self.snapped_shape:
                    ...
                    if self.current_stroke["type"] in ["pen", "highlighter", "eraser", "laser_pen"]: 
                        self.current_stroke["path"].lineTo(self.last_pos)
                        if "fill_path" in self.current_stroke: self.current_stroke["fill_path"].lineTo(self.last_pos)
                    else:
                        path = QPainterPath()
                        ...
                        if "fill_path" in self.current_stroke: self.current_stroke["fill_path"] = path # Geometric fill = stroke path
                
                if final_stroke["type"] == "pen" and len(final_stroke.get("pressures", [])) > 2:
                    final_stroke["path"] = self.generate_stroke_path(self.current_points, final_stroke["pressures"], final_stroke["size"])
                    final_stroke["is_variable"] = True
    ...
    def generate_stroke_path(self, points, pressures, base_size):
        path = QPainterPath()
        if len(points) < 2: return path
        left_side = []; right_side = []
        if len(pressures) < len(points): pressures.extend([pressures[-1]] * (len(points) - len(pressures)))

        for i in range(len(points) - 1):
            p1 = points[i]; p2 = points[i+1]
            pressure = (pressures[i] + pressures[i+1]) / 2
            width = pressure * base_size
            if width < 1.0: width = 1.0 
            
            vec = p2 - p1
            length = math.hypot(vec.x(), vec.y())
            if length == 0: continue
            norm = QPointF(-vec.y() / length, vec.x() / length)
            offset = norm * (width / 2.0)
            left_side.append(p1 + offset); right_side.append(p1 - offset)
            if i == len(points) - 2:
                left_side.append(p2 + offset); right_side.append(p2 - offset)

        if not left_side: return path
        path.moveTo(left_side[0])
        for p in left_side[1:]: path.lineTo(p)
        for p in reversed(right_side): path.lineTo(p)
        path.closeSubpath()
    ...
    def draw_stroke_entity(self, painter, stroke):
        ...
        # 1. DRAW FILL FIRST (Layer 0)
        if stroke.get("is_filled", False):
            # For variable strokes, use centerline. For others, use outline.
            fill_p = stroke.get("fill_path", path)
            # Create a closed copy for filling
            closed_fill = QPainterPath(fill_p)
            closed_fill.closeSubpath()
            
            painter.setPen(Qt.NoPen)
            painter.setBrush(stroke["fill_color"])
            painter.drawPath(closed_fill)

        # 2. DRAW STROKE (Layer 1)
        if stroke.get("is_variable", False):
        ...
        if st_type == "laser_pen":
            ...
            painter.setPen(glow_pen); painter.setCompositionMode(QPainter.CompositionMode_SourceOver); painter.setBrush(Qt.NoBrush); painter.drawPath(path)
            core_pen = QPen(Qt.white, max(1, stroke["size"] - 2), Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin)
            painter.setPen(core_pen); painter.drawPath(path)
            return
        ...
        if self.snapped_shape:
            ...
            if self.current_stroke["type"] != "text":
                if self.current_stroke["type"] not in ["pen", "highlighter", "eraser", "laser_pen"]:
                    ...
                    # Sync filled status for preview
                    temp_stroke["is_filled"] = self.current_stroke["is_filled"]
                    temp_stroke["fill_color"] = self.current_stroke["fill_color"]
                    if "fill_path" in self.current_stroke: temp_stroke["fill_path"] = self.current_stroke["fill_path"]
